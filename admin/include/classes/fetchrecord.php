...
class fetchrecord
{
  ...
  function ubstatus(){      
       ...
       $exid = $_GET['ubexid'];
        $result = $db->prepare("SELECT * FROM `ub_status` where examid=$exid");
            $result->execute();
            $row = $result->fetch();
            return $row;
  }
  ...
  function papercount()
  {
    include('config/dbconnection.php');
    $exid = $_GET['ubexid'];
    $result = $db->prepare("SELECT count(*) as cnt FROM `papermaster` where `examid`=$exid");
    $result->execute();
    $row = $result->fetch();
    return $row;
  }
  ...
  function getexam()
  {
    ...
    ?>
      <tr>
        ...
        <?php echo '<td><img src="' . $row['logo'] . '" class="img-fluid img-thumbnail mx-auto d-block"
       style="width: 60px;height:60px"/></td>'; ?>
        <!-- <td><img src="<?php //echo $row['logo'] 
                            ?>" style="width: 50px;height:50px"/> </td> -->
        <td>
          <button class="btn btn-primary" onclick="editexam(<?php echo $row['id'] ?>)">Edit</button>
          <button class="btn btn-warning" onclick="deleteexam(<?php echo $row['id'] ?>)">Delete</button>
          <!-- <button class="btn btn-info" onclick="configureexam(<?php //echo $row['id'] 
                                                                    ?>)">Configure</button> -->
          <a href="dashboard.php?ubexid=<?php echo $row['id']; ?>" target="_blank">
            <button class="btn btn-info">Configure</button></a>
        </td>
      </tr>
    <?php }
  }
  ...
  function pprdropdown()
  {
    $exid = $_GET['ubexid'];
    include('config/dbconnection.php');
    $result = $db->prepare("SELECT * from `papermaster` where `examid` = $exid ");
    $result->execute(); ?>
    <?php
    for ($j = 1; $row = $result->fetch(); $j++) {
