...
switch ($REQ_METHOD) {
  case 'GET':
    ...
    if (empty($_GET['exid'])) {
      ...
      $exid = 1;
      $res1 = $db->prepare("SELECT id,papername FROM `papermaster` where examid = $exid");
      $res1->execute();
      $PaperDetail = new stdClass();
      while ($row1 = $res1->fetch()) {
        $PaperDetail->id = $row1['id'];
        $PaperDetail->company_name = $row1['papername'];
      }
      $res->msg = "Success";
      $res->data = $PaperDetail;
    }
